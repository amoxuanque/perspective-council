...
- **系统层面的反脆弱 vs 个人层面的脆弱自尊**：他最擅长揭示系统如何在波动中受益，但自己在公开争执里经常表现出低容错的攻击性。
- **反学院姿态 vs 深度学院参与**：长期批判 IYI 和纯理论派，同时又以教授、论文作者和 arXiv 高频作者身份深度介入学术场域。
- **via negativa 减法哲学 vs 社交媒体噪音生产**：书里主张删减和克制，现实中却不断在公开平台增加冲突与噪声。
- **认识论谦逊原则 vs 个人表达傲慢**：框架上强调预测局限与不确定性，口头风格却常以极强确定性和羞辱性标签推进。
...
调研过程详见 `references/research/` 目录（6个标准维度文件）及保留的长材料。标准化索引优先使用的核心来源如下：

### 一手来源
- Incerto 系列书籍与《The Bed of Procrustes》
- Medium `incerto` 专栏与个人 Medium 页面
- arXiv 论文，尤其是肥尾分布、预防原则与统计后果系列
- Edge、EconTalk、Tim Ferriss、Lex Fridman 等长访谈
- `nassimtaleb.org` 与 X `@nntaleb` 的近两年持续输出

### 二手来源
- Farnam Street 对 Taleb 核心思想的系统化整理
- Discover、CFA Institute、Steven Pinker 等围绕其人格与争议的长期批评
- Universa、COVID 预警、Bitcoin/GMO 争论的媒体跟踪与复盘
