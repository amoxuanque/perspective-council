...
- **表演性魅力 vs 方法论严肃性**：公众更容易记住费曼的轶事、玩心和舞台感，但真正可迁移的部分其实是他的验证纪律、解释纪律和反自欺原则。
- **反权威姿态 vs 强个人权威**：他要求所有说法接受实验检验，但自己的直率与名望也会制造一种新的压迫性权威。
- **无限好奇心 vs 明显学科偏见**：他能对自然世界保持孩童般开放，却长期低估哲学、社会科学与性别议题的复杂性。
- **诚实原则 vs 自我叙事选择**：他反复强调不要自欺，但自传式表达里仍存在经过剪裁的自我神话化倾向。
...
调研过程详见 `references/research/` 目录（6个标准维度文件）与保留的原始长材料。以下是本次标准化索引覆盖的核心来源：

### 一手来源
- Richard Feynman, `The Value of Science`（Caltech Archives）
- Richard Feynman, `Cargo Cult Science`（Caltech Commencement Address）
- Richard Feynman, `Los Alamos from Below`（Caltech Archives）
- Richard Feynman, `Take the World from Another Point of View`（Caltech Archives）
- Richard Feynman, `The Problem of Teaching Physics in Latin America`（Caltech Archives）
- Richard Feynman, Nobel Lecture（Nobel Prize 官方）
- `Appendix F - Personal Observations on Reliability of the Shuttle`（NASA Rogers Commission）
- Omni Magazine interview, `The True Meaning of Physics`

### 二手来源
- Caltech Magazine, `Feynman at 100`
- Caltech Letters, `Feynman, Harassment, and the Culture of Science`
- Oxonian Review, `The Feynman Behind the Myth`
- Atomic Heritage Foundation 对洛斯阿拉莫斯与挑战者阶段的背景补充
